		...
		<!-- <div class="row"> -->
			...
			?>
				<tr>
					...
					<td><a href="?mode=form&jenis=pelamar&id=<?=$arr['id_event']?>&t=<?=md5('term')?>" class="btn btn-xs btn-info">Edit</a> || <a href="modul/hapus.php?mode=hapus_event&id=<?=$arr['id_event']?>" onclick="return confirm('Anda Yakin Ingin Menghapus Data Ini?')" class="btn btn-xs btn-danger">Hapus</a></td>
				</tr>
			<?php $no++; } ?>
			</tbody>
		</table>
  ...
  templates: [{
	...
	<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">
		...
		<!-- <div class="row"> -->
			...
			<?php
				if(isset($_GET['t'])){
					$sql1="Select * from event where id_event=$_GET[id]";
					$res1=mysql_query($sql1);
					$arr1=mysql_fetch_array($res1);
					echo "<form action=\"modul/update.php?mode=update_event&id=$_GET[id]\" method=\"POST\" enctype=\"multipart/form-data\">";
				}else{
					echo "<form action=\"modul/simpan.php?mode=simpan_event\" method=\"POST\" enctype=\"multipart/form-data\">";
				}
			?>
			
			<!-- <form action="modul/simpan.php?mode=simpan_event" method="POST" enctype="multipart/form-data"> -->
				...
				<tr>				
					...
					<td>:</td>
					<td>
					<textarea  name="isian" class="form-control" style="width: 50%;background-color: #d6e5ff;color: #002f82"> <?php if(isset($_GET['t'])){ echo "$arr1[isi]"; }?></textarea></td>
				</tr>
				<tr>				
				...
				<?php
					if(isset($_GET['t'])){
						echo "<tr>
							...
							<td></td>
							<td colspan='3'>
								<img src='event/$arr1[gambar]' style=width:50%>
				...
				<tr>				
					<td width="10%">Tanggal Event</td>
					<td>:</td>
					<td>
					<input type="text" style="height: 30px;width: 200px;background-color: #d6e5ff;color: #002f82" id="datetimepicker1" value="<?php if(isset($_GET['t'])){ echo "$arr1[tanggal]"; }?>" name="tanggal" class="form-control" max-length="10"  readonly>
					</td>
				</tr>
