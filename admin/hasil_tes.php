		...
		<!-- <div class="row"> -->
			...
			?>
				<tr>	
					...
					<td><a onclick="window.open('?mode=lihat2&judul=<?=$arr['judul']?>&id=<?=$arr['id_lowongan']?>','List','Width=700,height=500')"><?=$arr['judul']?></a></td>
					<td><?=$cetak?></td>
					<td><?=$arr['provinsi']?></td>
					<td><?=$arr1['banyak']?></td>
					<!--  -->
...
<script type="text/javascript" src="//cdn.datatables.net/1.10.15/js/jquery.dataTables.min.js"></script>
	<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">
		...
		<!-- <div class="row"> -->
			...
			<?php
				$no=1;
				$sql="SELECT nama,tempat_lahir,pendidikan,alamat,telepon,tgl_lahir,level_pendidikan,a.id_user as user_id,file,panggilan FROM tbl_lamar a inner join user b on a.id_user=b.id_user LEFT join tbl_cv c on b.id_user=c.id_user where id_lowongan='$_GET[id]' AND a.panggilan='1'" ;
				// echo $sql;
				$res=mysql_query($sql);
				while($arr=mysql_fetch_array($res)){
				$bgcolor=$no%2;
